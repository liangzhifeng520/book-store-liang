...
class OrderPlaceView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        for sku_id in sku_ids:
            ...
            # 计算商品的小计
            amount = int(sku.price * int(count))
...
class OrderCommitView1(View):
    ...
    def post(self, request):
        ...
        # 组织参数
        # 订单id: 20191122181630+用户id  datetime.now()时间函数
        ...
        try:
            ...
            for sku_id in sku_ids:
                # 获取商品的信息
                try:
                    #加一个悲观锁，防止并发出现订单相同问题  for update加锁（悲观锁）
                    # select * from df_goods_sku where id=sku_id for update;
                    sku = GoodsSKU.objects.select_for_update().get(id=sku_id)
                except:
                    # 商品不存在
                    transaction.savepoint_rollback(save_id)   #回滚事务，让事件取消
                    return JsonResponse({'res':4, 'errmsg':'商品不存在'})

                # 从redis中获取用户所要购买的商品的数量
                count = conn.hget(cart_key, sku_id)

                #todo:判断商品的库存（以免同时下单数量超了）
...
class OrderCommitView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            sku_ids = sku_ids.split(',')
            for sku_id in sku_ids:
                for i in range(3): #重新读取3次，防止mysql事务获取不到更新后数据
                    # 获取商品的信息
                    try:
                        sku = GoodsSKU.objects.get(id=sku_id)
                    except:
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        alipay = AliPay(
            appid="2016102500755360", # 应用id
            app_notify_url=None,  # 默认回调url
            app_private_key_string=app_private_key_string,  #django2.0用app_private_key_string
            alipay_public_key_string=alipay_public_key_string, # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
            sign_type="RSA2",  # RSA 或者 RSA2 （加密算法）
            debug=True  # 默认False
        )
